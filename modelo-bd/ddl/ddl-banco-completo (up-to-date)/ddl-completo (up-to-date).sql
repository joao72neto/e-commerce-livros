...
CREATE TABLE IF NOT EXISTS `e_commerce_books`.`trocas` (
  ...
  `trc_status` ENUM('Troca Solicitada', 'Troca Aceita', 'Troca Concluída', 'Troca Recusada', 'Devolução Solicitada', 'Devolução Aceita', 'Devolução Concluída', 'Devolução Recusada') NULL,
